...
{ op := tree[x + t_op];
  ...
  if (op = s_ass) or (symbol = s_input) or (symbol = s_output)
  then
  { tree[x + t_op2] := optimiseexpr(tree[x + t_op2]);
    tree[x + t_op1] := optimiseexpr(tree[x + t_op1])
  }
...
proc gentabvals(val x) is
  if tree[x + t_op] = s_comma
  then
  { gentabvals(tree[x + t_op1]);
    gentabvals(tree[x + t_op2])
  }
  else
  if isval(x)
  then
...
{ count := 0;
  ...
  { outlab('S', count);
    ...
    while b < bytes do
    { prints("  DATA    ");
      printn(rem(strings[count], 65536)); 
      newline();
      b := b + 2;
      if b < bytes 
      then 
      { prints("  DATA    ");
        printn(div(strings[count], 65536)); 
        newline();
        b := b + 2
      }
      else
